...
export default function OrderDetailsDrawer({ order, onClose }) {
  ...
  const fullName = user?.fullName || order.guest_name || t('profile.orderDetails.guest');
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      <button
        type="button"
        aria-label={t('profile.orderDetails.closeAriaLabel')}
        onClick={onClose}
      ...
      <div className="relative flex h-full w-full max-w-md flex-col overflow-y-auto rounded-s-[2rem] border-s-4 border-carissma-400 bg-carissma-50 p-6 shadow-2xl sm:p-8">
        ...
        <div className="mt-6 flex items-start justify-between gap-4">
          ...
          <div className="text-end">
            <p className="text-sm font-bold text-espresso-600">{t('profile.orderDetails.orderId')}</p>
        ...
        <div className="mt-6 grid grid-cols-2 gap-x-4 gap-y-5 border-t border-carissma-200 pt-5 text-sm">
          <div>
            <p className="font-bold text-espresso-600">{t('profile.orderDetails.fullName')}</p>
            <p className="mt-1 font-extrabold text-espresso-900">{fullName}</p>
          </div>
          <div className="text-end">
            <p className="font-bold text-espresso-600">{t('profile.orderDetails.paymentMethod')}</p>
            <p className="mt-1 font-extrabold text-espresso-900">
              {PAYMENT_METHOD_KEYS[order.payment_method] ? t(PAYMENT_METHOD_KEYS[order.payment_method]) : order.payment_method || '—'}
            </p>
          </div>
          <div>
            <p className="font-bold text-espresso-600">{t('profile.orderDetails.phoneNumber')}</p>
            <p className="mt-1 font-extrabold text-espresso-900">{phone}</p>
          </div>
          <div className="text-end">
            <p className="font-bold text-espresso-600">{t('profile.orderDetails.date')}</p>
            <p className="mt-1 font-extrabold text-espresso-900">{formatOrderDate(order.created_at)}</p>
          </div>
          <div>
            <p className="font-bold text-espresso-600">{t('profile.orderDetails.address')}</p>
            <p className="mt-1 font-extrabold text-espresso-900">{address || '—'}</p>
          </div>
          <div className="text-end">
            <p className="font-bold text-espresso-600">{t('profile.orderDetails.orderStatus')}</p>
            <p className="mt-1 font-extrabold text-espresso-900">{t(meta.labelKey)}</p>
          </div>
        </div>

        <p className="mt-6 text-base font-extrabold text-espresso-900">{t('profile.orderDetails.myOrder')}</p>
        <div className="mt-3 space-y-3">
          ...
          {items.length === 0 && <p className="text-sm font-semibold text-espresso-400">{t('profile.orderItem.noItems')}</p>}
        </div>

        <div className="mt-5 space-y-2.5 rounded-2xl bg-white/70 p-5 text-sm">
          <div className="flex items-center justify-between">
            <span className="font-bold text-espresso-900">{t('shop.cart.subtotal')}</span>
            <span className="font-extrabold text-espresso-900">{formatPrice(order.subtotal, currency)}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="font-bold text-carnation-600">{t('shop.cart.discount')}</span>
            <span className="font-extrabold text-carnation-600">{discountPercent(order)}%</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="font-bold text-espresso-900">{t('shop.cart.deliveryFees')}</span>
          ...
          <div className="flex items-center justify-between border-t border-carissma-200 pt-2.5">
            <span className="font-bold text-carissma-400">{t('shop.cart.total')}</span>
