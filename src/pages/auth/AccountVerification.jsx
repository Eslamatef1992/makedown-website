...
export default function AccountVerification() {
  ...
  return (
    <AuthLayout title={t('auth.accountVerification.title')}>
      <div className="space-y-2">
        <span className="block text-sm font-bold text-espresso-900">{t('auth.accountVerification.yourEmail')}</span>
        <div className="flex items-center justify-between rounded-2xl border border-carissma-200 bg-carissma-100/70 px-4 py-3">
          ...
          >
            {t('auth.accountVerification.changeEmail')}
          </Link>
        </div>
      </div>

      <p className="mt-4 text-sm font-semibold text-espresso-800">
        {t('auth.accountVerification.body')}
      </p>
      ...
      <Button type="button" onClick={onSendOtp} loading={loading} className="mt-6">
        {t('auth.accountVerification.sendOtp')}
      </Button>
    </AuthLayout>
  );
}
